...
fn copy_fixture(name: &str) -> PathBuf {
    ...
    let deep = root.join("src/deep/unrelated");
    std::fs::create_dir_all(&deep).expect("create project directories");
    copy("osprey.toml", &root.join("osprey.toml"));
    copy("src/main.osp", &root.join("src/main.osp"));
    copy("src/cream.ospml", &root.join("src/cream.ospml"));
    copy("src/deep/unrelated/tax.osp", &deep.join("tax.osp"));
    root
}
...
fn directory_and_manifest_inputs_share_the_flat_project_pipeline() {
    ...
    assert!(
        symbols.stdout.contains("billing/api::Tax::addTax"),
        "{}",
        symbols.stdout
    );
    assert!(!symbols.stdout.contains("__osp_"), "{}", symbols.stdout);
    assert_project_main_symbol_is_local(&symbols.stdout, &project.join("src/main.osp"));
}
...
fn assert_project_main_symbol_is_local(json: &str, source: &Path) {
    ...
    let main = value.as_array().and_then(|entries| {
        entries.iter().find(|entry| {
            entry.get("name").and_then(serde_json::Value::as_str) == Some("app::main")
        })
    });
    let expected_line = std::fs::read_to_string(source)
        .expect("read entry source")
        .lines()
        .position(|line| line.trim_start().starts_with("fn main"))
...
#[test]
fn mixed_flavor_path_independent_project_runs() {
    let project = fixture();
    let output = run(&[arg(&project), "--run".to_string()]);
    assert_eq!(output.code, Some(0), "stderr={}", output.stderr);
    assert_eq!(output.stdout, "tax=110 counter=1/2\n");
}
...
fn link_directives_from_a_non_entry_source_reach_the_native_driver() {
    ...
    let module = project.join("src/deep/unrelated/tax.osp");
...
fn build_subcommand_uses_project_name_inside_project_directory() {
    ...
    let artifact = project.join("modules");
...
fn flattened_type_errors_map_back_to_physical_local_lines() {
    ...
    let cream = project.join("src/cream.ospml");
    let source = std::fs::read_to_string(&cream).expect("read ML source");
    let broken = source.replace(
        "export label cents = \"tax=${taxApi::Tax::addTax cents}\"",
        "export label : int -> int\n    label cents = \"tax=${taxApi::Tax::addTax cents}\"",
    );
    assert_ne!(broken, source, "fixture mutation must change the ML source");
    let line = broken
        .lines()
        .position(|line| line.contains("label cents ="))
        .map_or(0, |line| line.saturating_add(1));
    std::fs::write(&cream, broken).expect("write broken ML source");

    let output = run(&[arg(&project), "--check".to_string()]);
    assert_ne!(output.code, Some(0));
    let expected = format!("{}:{line}:", cream.display());
