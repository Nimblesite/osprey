...
use crate::testutil::{accepts, ctor_arity_message, rejects_with, spec_cases};
...
spec_cases! {
    ...
    a_binder_spans_every_variant_field: accepts(Default, r#"type Holder<T, U> = Full { left: T, right: U } | Half { only: T } | None
...
print(label(Full { left: 1, right: "one" }))"#);
    ...
    one_binder_used_twice_pins_both_fields: rejects_with(Default, r#"type Both<T> = { left: T, right: T }
let b = Both { left: 1, right: "two" }
print("${b.left}")"#, "cannot unify");

    /// "A construction site may apply explicit type arguments … which unify with
    /// the instantiation the fields would otherwise infer."
    a_construction_site_may_pin_its_instantiation: accepts(Default, format!(
            r#"{PAIR}let p = Pair<int, string> {{ first: 1, second: "a" }}
print("${{p.first}}${{p.second}}")"#
        ));

    /// "an argument that contradicts a field is a type error."
    a_construction_type_argument_contradicting_a_field_is_rejected: rejects_with(Default, format!(
            r#"{BOX}let b = Box<int> {{ value: "text" }}
print("${{b.value}}")"#
        ), "cannot unify");

    /// The contradiction is caught in the second position as well.
    a_contradiction_in_the_second_type_argument_is_rejected: rejects_with(Default, format!(
            r#"{PAIR}let p = Pair<int, int> {{ first: 1, second: "a" }}
print("${{p.first}}")"#
        ), "cannot unify");

    /// A nested instantiation is pinned the same way.
    a_nested_construction_type_argument_is_checked: accepts(Default, format!(
...
spec_cases! {
    ...
    too_many_construction_type_arguments_are_rejected: rejects_with(Default, format!(
            r#"{BOX}let b = Box<int, string> {{ value: 1 }}
print("${{b.value}}")"#
        ), ctor_arity_message("Box", 1, 2));

    /// The other direction: two binders, one written argument.
    too_few_construction_type_arguments_are_rejected: rejects_with(Default, format!(
...
print("${{p.first}}")"#
    ...
    a_non_generic_type_takes_no_construction_type_arguments: rejects_with(Default, r#"type Plain = { value: int }
let p = Plain<int> { value: 1 }
print("${p.value}")"#, ctor_arity_message("Plain", 0, 1));

    /// Omitting the arguments entirely stays legal — they are optional, and
    /// inference supplies them.
    omitting_construction_type_arguments_still_infers: accepts(Default, format!(
...
spec_cases! {
    ...
    an_unbound_annotation_name_is_nominal_not_a_binder: rejected_somehow(Default, r#"fn nom(a: T) -> T = a
print("${nom(1)}")"#);
}
...
spec_cases! {
    ...
    /// polymorphic."
    an_unannotated_function_stays_implicitly_polymorphic: accepts(Default, r#"fn id(x) = x
print("${id(5)} ${id("os")} ${toString(id(true))}")"#);

    /// Two binders are independent: they may be instantiated at the same type or
    /// at different ones.
    two_binders_are_independent: accepts(Default, r#"fn pair<T, U>(first: T, second: U) -> T = first
print("${pair(1, "a")} ${pair(1, 2)}")"#);

    /// A binder list names DISTINCT parameters; a repeat has no meaning.
    a_repeated_binder_name_is_rejected: rejected_somehow(Default, r#"fn dup<T, T>(first: T, second: T) -> T = first
print("${dup(1, 2)}")"#);

    /// An empty binder list is not a binder list.
    an_empty_binder_list_is_rejected: rejected_somehow(Default, r#"fn none<>(x) = x
print("${none(1)}")"#);

    /// A binder may appear in the return position only — the shape call-site type
    /// application exists to serve ([TYPE-GENERICS-APPLY]).
    a_binder_may_appear_in_the_return_position_only: accepts(Default, r#"fn empty<T>() -> List<T> = []
...
spec_cases! {
    ...
    a_generic_function_binds_as_a_call_alias: accepts(Default, r#"fn identity<T>(x: T) -> T = x
let g = identity
print("${g(5)} ${g("os")}")"#);

    /// …and the consuming-slot form: a generic function passed by name to a HOF.
    a_generic_function_specialises_into_a_consuming_slot: accepts(Default, r#"fn identity<T>(x: T) -> T = x
fn also(x, f) = f(x)
print("${also(5, identity)} ${also("os", identity)}")"#);
    ...
    a_returned_generic_lambda_serves_every_call_site_of_its_binding: accepts(Default, r#"fn mk() = |x| => x
let f = mk()
print("${f(7)} ${f("os")} ${f(2.5)}")"#);

    /// "A lambda so returned may close over the producing call's parameters."
    a_returned_lambda_may_close_over_the_producing_call: accepts(Default, r#"fn constly(v) = |x| => v
...
print("${always(1)} ${always("os")}")"#);
    ...
    a_still_generic_lambda_as_a_bare_value_is_rejected: rejects_with(Default, r#"fn mk(x) = |y| => x
print("${mk(1)}")"#, "a closure value with a still-generic type");
}
...
spec_cases! {
    ...
    ml_type_binders_are_juxtaposed: accepts(Ml, "type Box T =\n    value : T\nheld = Box(value = 7)\nprint \"${held.value}\"\n");

    /// "Construction-site type arguments use `Box<int>(item = 7)`."
    ml_construction_type_arguments_use_angles: accepts(Ml, "type Box T =\n    value : T\nheld = Box<int>(value = 7)\nprint \"${held.value}\"\n");

    /// The ML construction site is held to the same arity contract.
    ml_construction_type_argument_arity_is_checked: rejects_with(Ml, "type Box T =\n    value : T\nheld = Box<int, string>(value = 7)\nprint \"${held.value}\"\n", ctor_arity_message("Box", 1, 2));

    /// "Function binders appear on a signature: `pick<T> : (T, T) -> T`."
    ml_function_binders_live_on_the_signature: accepts(Ml, "pick<T> : (T, T) -> T\npick (first, second) = first\nkept = pick (10, 20)\nprint \"${kept}\"\n");

    /// "A binding without a signature cannot declare function type parameters."
    ml_a_binding_cannot_declare_type_parameters_without_a_signature: rejected_somehow(Ml, "pick<T> (first, second) = first\nkept = pick (10, 20)\nprint \"${kept}\"\n");

    /// The ML twin of the one-binder-relates-two-parameters rule.
    ml_one_binder_relates_two_parameters: rejects_with(Ml, "pick<T> : (T, T) -> T\npick (first, second) = first\nkept = pick (10, \"twenty\")\nprint \"${kept}\"\n", "cannot unify");
}
