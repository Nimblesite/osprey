...
use crate::testutil::{accepts, check, rejected_somehow, spec_cases};
use osprey_syntax::Flavor;

spec_cases! {
    /// A function type inside a covariant built-in still flips its parameter.
    out_in_a_function_parameter_inside_a_list_is_rejected: rejects_with(Default, decl("<out T>", "checks", "List<(T) -> int>"), position_message("T", "out", "input", "checks", "Holder"));

    /// The `Result` ERROR channel is an output position too.
    out_is_legal_in_the_result_error_channel: accepts(Default, decl("<out T>", "failure", "Result<int, T>"));

    /// …so a contravariant parameter is rejected there.
    in_in_the_result_error_channel_is_rejected: rejects_with(Default, decl("<in T>", "failure", "Result<int, T>"), position_message("T", "in", "output", "failure", "Holder"));

    /// `Fiber<out T>` is an output position, so `in T` cannot sit there.
    in_inside_a_fiber_is_rejected: rejects_with(Default, decl("<in T>", "worker", "Fiber<T>"), position_message("T", "in", "output", "worker", "Holder"));

    /// `Map<K, out V>`: a contravariant parameter is rejected in the VALUE channel.
    in_in_a_map_value_is_rejected: rejects_with(Default, decl("<in T>", "byName", "Map<string, T>"), position_message("T", "in", "output", "byName", "Holder"));
}
...
spec_cases! {
    ...
    a_parameter_used_in_both_positions_may_be_invariant: accepts(Default, both_positions("<T>"));

    /// The covariant spelling is rejected on the input half.
    a_parameter_used_in_both_positions_may_not_be_covariant: rejects_with(Default, both_positions("<out T>"), position_message("T", "out", "input", "consume", "Holder"));

    /// The contravariant spelling is rejected on the output half.
    a_parameter_used_in_both_positions_may_not_be_contravariant: rejects_with(Default, both_positions("<in T>"), position_message("T", "in", "output", "held", "Holder"));
}
...
spec_cases! {
    ...
    mixed_markers_are_checked_per_parameter: accepts(Default, "type Two<out A, in B> = { give: A, take: (B) -> bool }\nprint(\"declared\")");
}
...
spec_cases! {
    ...
    out_is_legal_in_a_union_variant_payload: accepts(Default, "type Maybe<out T> = Some { value: T } | None\nprint(\"declared\")");

    /// …so a contravariant parameter is rejected there.
    in_in_a_union_variant_payload_is_rejected: rejects_with(Default, "type Maybe<in T> = Some { value: T } | None\nprint(\"declared\")", position_message("T", "in", "output", "value", "Maybe"));
}
...
spec_cases! {
    ...
    a_violation_in_a_later_variant_is_still_found: rejects_with(Default, "type Split<out T> = First { ok: T } | Second { consume: (T) -> int } | Empty\nprint(\"declared\")", position_message("T", "out", "input", "consume", "Split"));
}
