...
static void t_scope_forwarding(void) {
  CHECK(__osprey_handler_push_scoped("State", "get", (void *)fn_a, NULL, 0) == 0);
  ...
  CHECK(__osprey_handler_lookup_env("Log", "write") == &env_b);
  CHECK(__osprey_handler_lookup("Arm", "local") == NULL);
...
static void t_scope_snapshot_preserves_activation(void) {
  ...
  HandlerScope *scope = __osprey_handler_suspend_scope("State", "put");
  CHECK(__osprey_handler_depth() == 1);
  CHECK(__osprey_handler_lookup("State", "get") == (void *)fn_a);
...
static void death_scope_missing_handler(void) {
  (void)__osprey_handler_suspend_scope("Missing", "operation");
}
...
static int64_t body_suspending_scope(void *raw) {
  ...
  __osprey_handler_restore_scope(scope);
  return value + (__osprey_handler_lookup("State", "get") == (void *)fn_b);
}
...
static void t_handler_scope(void) {
  ...
  CHECK(__osprey_handler_push_scoped("State", "get", (void *)fn_a, NULL, -1) == -1);
  CHECK(__osprey_handler_push_scoped("State", "get", (void *)fn_a, NULL, 1) == -1);
  CHECK(__osprey_handler_depth() == 0);
