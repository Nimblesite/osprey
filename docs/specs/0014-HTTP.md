...
> **Flavor layer — shared core (AST and above).** HTTP semantics are flavor-blind after lowering to `osprey_ast::Program`; the runtime cannot observe the source flavor. Examples use Default named-argument calls; see [ML Flavor Syntax](0024-MLFlavorSyntax.md) and [Language Flavors](0023-LanguageFlavors.md).
