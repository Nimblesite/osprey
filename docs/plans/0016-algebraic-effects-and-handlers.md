...
| Editor and docs row display | Written function-row bounds survive inference in hover, signature help, completion detail, `--symbols` and API pages, in both flavors. The shared signature renders the row once. Inferred transitive required-operation and handler-instance views remain open. | `crates/osprey-lsp/src/inferred_views.rs`, `crates/osprey-cli/tests/cases/api_docs_extra.rs`, `docs/specs/0020-LanguageServerAndEditors.md` |
