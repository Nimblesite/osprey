...
| Effects checker | Closed-program operation propagation, partial generic discharge and explicit closed empty `![]` bounds exist in both flavors, including ascribed module signatures. Independently quantified open rows do not. | `crates/osprey-types/src/effect_rows.rs`, `generic_effects_tests.rs`, `effect_rows_tests.rs`, `crates/osprey-project/tests/module_diagnostics.rs` |
