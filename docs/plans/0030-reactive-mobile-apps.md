...
- [ ] Re-run the smoke on the physical iPhone now that Markdown rendering has landed. On 2026-09-10, a direct `devicectl device info details` query reconnected the iPhone 16 and reported developer services available. The current Osprey iOS C ABI archive compiled successfully, but Xcode's signed device build then timed out because the developer disk image was not mounted; its diagnostic requires the phone to be unlocked. The updated physical smoke has therefore still not executed. The earlier signed physical-device smoke remains recorded separately.
