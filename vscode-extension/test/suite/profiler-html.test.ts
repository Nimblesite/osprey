...
import {
  ...
  makeNonce,
  pathBasename,
} from "../../client/src/profiler/flame-html";
import {
  buildFlameModel,
  type SpeedscopeFile,
...
import {
  clampViewRange,
  fitLabelText,
} from "../../client/src/profiler/flame-script-helpers";
...
suite("flame-html document structure", () => {
  ...
  test("embeds the model as a JSON blob with < escaped", () => {
    assert.ok(
      html.includes(
        `<script type="application/json" id="flame-data" nonce="${NONCE}">`,
      ),
    );
    assert.ok(html.includes('"fibers"') && html.includes('"leftHeavy"'));
    assert.ok(
      html.includes("evil\\u003c/script>"),
      "frame names cannot close the data blob",
    );
  });

  test("escapes the title and includes the inline renderer", () => {
    assert.ok(
      html.includes("<title>Profile: fib&lt;&amp;&gt;&quot;osp</title>"),
    );
...
suite("flame-html header and fiber controls", () => {
  test("header strip formats the run summary", () => {
    assert.ok(
      html.includes("4182 samples · 997Hz · 4.2s wall · 3.9s CPU · 3 fibers"),
    );
  });
  ...
  test("per-fiber chips show on-CPU share; unsampled fibers get no chip", () => {
    assert.ok(
      html.includes(
        '<button class="chip" data-fiber="0">main · 90% on-CPU</button>',
      ),
    );
    ...
    assert.ok(
      !html.includes("fiber-2 · 0% on-CPU"),
      "zero-sample fibers must not render chips",
    );
  ...
  test("a zero-sample fiber row between sampled ones does not shift chip indices", () => {
    ...
    assert.strictEqual(
      model.fibers[1].name,
      "beta",
      "chip 1 selects model fiber 1 = beta",
    );
  });
  ...
  test("the fiber select lists each speedscope profile by name", () => {
    assert.ok(
      html.includes(
        '<select id="fiber-select"><option value="0">main</option></select>',
      ),
    );
  });
});
...
suite("flame-html hot functions table", () => {
  ...
  test("fileless runtime rows carry no navigation and show a dash", () => {
    assert.ok(
      html.includes(
        '<tr><td class="num">5.0</td><td class="num">5.5</td><td>memcpy</td><td class="loc">—</td></tr>',
      ),
    );
  });
});
...
suite("flame-script interactions", () => {
  test("wires zoom, pan, hover, select, search, and reset", () => {
    for (const needle of [
      ...
      assert.ok(
        FLAME_SCRIPT.includes(needle),
        `script should include ${needle}`,
      );
    }
  });
...
suite("flame-script pure helpers", () => {
  ...
  test("clampViewRange enforces span and [0,1] bounds", () => {
    assert.deepStrictEqual(clampViewRange(0.25, 0.75, 0.001), {
      x0: 0.25,
      x1: 0.75,
    });
    assert.deepStrictEqual(clampViewRange(-0.25, 0.25, 0.001), {
      x0: 0,
      x1: 0.5,
    });
    assert.deepStrictEqual(clampViewRange(0.75, 1.5, 0.001), {
      x0: 0.25,
      x1: 1,
    });
    assert.deepStrictEqual(clampViewRange(-1, 2, 0.001), { x0: 0, x1: 1 });
    assert.deepStrictEqual(clampViewRange(0.5, 0.5, 0.25), {
      x0: 0.5,
      x1: 0.75,
    });
  });
