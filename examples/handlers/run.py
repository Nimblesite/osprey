...
STAGED_ROW_LANGUAGES = ("osprey", "osprey-ml")
...
def main():
    ...
    parser.add_argument("--demo", default="handlers", choices=("handlers", "semantics", "returns", "open-rows", "staged-rows"))
    parser.add_argument("--check", action="store_true", help="assert the checked-in expected output")
    args = parser.parse_args()
    BUILD.mkdir(parents=True, exist_ok=True)
    available = {"open-rows": OPEN_ROW_LANGUAGES,
                 "staged-rows": STAGED_ROW_LANGUAGES}.get(args.demo, LANGUAGES)
